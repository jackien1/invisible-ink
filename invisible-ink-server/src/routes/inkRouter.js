...
router.post(
  ...
  async (req, res) => {
    ...
    const search = schools.map(async school => {
      let schoolInstance = new web3.eth.Contract(School.abi, school);
      let inks = await schoolInstance.methods.returnInks().call({
        from: user.address
      });
      ...
      const finishedResult = await Promise.all(result);
      return { address: school, inks: finishedResult };
    });

    const allInks = await Promise.all(search);
